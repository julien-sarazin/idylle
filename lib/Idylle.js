...
class Idylle {
    ...
    init() {
        ...
        function initDependencies() {
            return load(Idylle.events.init.dependencies)
                ...
                .then(setCriteriaBuilderToActions);
            ...
            function setDependencies(dependencies) {
                ...
                if (dependency) {
                    ...
                    if (dependency.server && typeof dependency.server !== 'function')
                        throw new InvalidServerError(dependency.server);
                    self.server = dependency.server;
                    self.Router = dependency.Router;
                    return;
                }

                self.criteriaBuilder = require('./CriteriaBuilder');
                self.errorHandler = require('./ErrorHandler');
                self.server = express();
                self.Router = express.Router;
            }

            function setCriteriaBuilderToActions() {
                Action.criteriaBuilder = new self.criteriaBuilder();
            }
        }

        function setup() {
            self.server.use(decorateResponse);
        ...
        function start() {
            return self.server.listen(self.settings.port);
        }
    ...
    on(event, listener) {
        ...
        else if (event === Idylle.events.init.models)
            this.add(listener, event, this.models);

        else if (event === Idylle.events.init.cache)
            this.add(listener, event, this.cache);
