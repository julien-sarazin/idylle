...
class Idylle {
    ...
    on(event, listener) {
        ...
        if (event === Idylle.events.init.settings)
            return add(listener, event, this.settings);

        if (event === Idylle.events.init.models)
            return add(listener, event, this.models);

        if (event === Idylle.events.init.criteriaBuilder)
            return add(listener, event, this.criteriaBuilder);

        if (event === Idylle.events.init.cache)
            return add(listener, event, this.cache);

        add(listener, event, this);


        function add(listener, onEvent, withObject) {
