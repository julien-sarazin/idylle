    ...
    init() {
        ...
        function initSettings() {
            return load(Idylle.events.init.settings)
                .then(loaded => (loaded.length) ? undefined : imports('./settings', {args: self.settings}).then(s => self.settings = s || {port: DEFAULT_PORT, host: DEFAULT_HOST}))
        }
        ...
        function initCache() {
            return load(Idylle.events.init.cache)
                .then(loaded => (loaded.length) ? undefined : self.cache = imports('./cache', {args: self}))
        }
